...
class CartController {
    ...
    async registerProductCart(request, response) {
        try {
            ...
            const result = await this._service.registerProductCart(user_id, quantity, product_id, product, unit_price, total_price, order_id, status);
            response.status(200).json({ result });
        }
        catch (error) {
            response.status(500).json({ error: error.message || error.toString() });
    ...
    async updateProductCart(request, response) {
        try {
            ...
            const total_price = request.params.total_price;
            const status = request.params.status;
            const result = await this._service.updateProductCart(id, user_id, quantity, product_id, product, unit_price, total_price, status);
            response.status(200).json({ result });
        }
        catch (error) {
            response.status(500).json({ error: error.message || error.toString() });
