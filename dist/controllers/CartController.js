...
class CartController {
    ...
    deleteProductCart(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const _id = request.params.id;
                const result = yield this._service.deleteProductCart(_id);
                response.status(200).json({ result });
            }
            catch (error) {
                response.status(500).json({ error: error.message || error.toString() });
    ...
    registerProductCart(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const result = yield this._service.registerProductCart(user_id, quantity, product_id, product, unit_price, total_price, order_id, status);
                response.status(200).json({ result });
            }
            catch (error) {
                response.status(500).json({ error: error.message || error.toString() });
    ...
    updateProductCart(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const total_price = request.params.total_price;
                const status = request.params.status;
                const result = yield this._service.updateProductCart(id, user_id, quantity, product_id, product, unit_price, total_price, status);
                response.status(200).json({ result });
            }
            catch (error) {
                response.status(500).json({ error: error.message || error.toString() });
